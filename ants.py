...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        mastermind = self.place.exit
        while mastermind is not None:  
            if mastermind.ant is not None:
                if mastermind.ant.is_container == True:
                    if mastermind.ant.ant_contained is not None:
                        if mastermind.ant.ant_contained.damage_double == False:
                            mastermind.ant.ant_contained.damage *= 2
                            mastermind.ant.ant_contained.damage_double = True
                if mastermind.ant.damage_double == False:
                    mastermind.ant.damage *= 2
                mastermind.ant.damage_double = True
            mastermind = mastermind.exit
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        k = 0
        current = self.place
        while current.is_hive != True:
            if current.bees != [] and self.lower_bound <= k <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        fake_bees = list(self.place.bees)
        Ant.reduce_health(self, amount)
        if fake_bees != []:
            if self.health <= 0:
                amount += self.damage
                for slays in fake_bees:
                    Insect.reduce_health(slays, amount)
            else:
                for slays in fake_bees:
                    Insect.reduce_health(slays, amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            kill_it_with_fire = random_bee(self.place.bees)
            if kill_it_with_fire != None:
                Insect.reduce_health(kill_it_with_fire, kill_it_with_fire.health)
...
class ContainerAnt(Ant):
    """
    ContainerAnt can share a space with other
    """
    is_container = True
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        noms = list(self.place.bees)
        if noms != []:
            for chomps in noms:
                chomps.reduce_health(self.damage)
# END Problem 9
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if cls.queen_exists == 0:
            cls.queen_exists += 1
            cls.is_queen = True
            cls.midnights = super().construct(gamestate)
            return cls.midnights
        else:
            return None
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def apply_status(self, status, bee, turns):
        normal_action = bee.action
        effected_action = self.make_slow(bee.action, bee)
        def action(gamestate):
            if self.turns > 0:
                self.make_slow(action, bee)
                self.turns -= 1
            else:
                bee.action(gamestate)
                
                
    def make_slow(self, action, bee):
        def slow(gamestate):
    ...
    def throw_at(self, target):
        if target:
            self.apply_status(self.make_slow, target, 5)
