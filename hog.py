...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        while who == 0 and score1 < goal:
            slay = strategy0(score0, score1)
            score0 += take_turn(slay, score1, dice)
            if perfect_square(score0) and update == square_update:
                score0 = next_perfect_square(score0)
            if score0 >= goal:
                return score0, score1
            beyonce = update(slay, score0, score1)
            who = 1 - who
           
        while who == 1 and score0 < goal:
            serve = strategy1(score1, score0)
            score1 += take_turn(serve, score0, dice)
            if perfect_square(score1) and update == square_update:
                score1 = next_perfect_square(score1)
            if score1 >= goal:
                return score0, score1
            rihanna = update(serve, score0, score1)
            who = 1 - who
...
def make_averaged(original_function, total_samples=1000):
    ...
    def baka(*args):
        ...
        while k < total_samples:
            sussy += original_function(*args)
            k = k + 1
        return (sussy / total_samples)
    return baka
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    l = 1
    while k <= 10: 
        howdareyou = make_averaged(roll_dice, total_samples)(k, dice)
        if howdareyou > imhungry:
            imhungry = howdareyou
            l = k
        k += 1
    return l 
